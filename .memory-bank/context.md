...
## Current Phase
**Early Prototype — All 17 systems COMPLETE + UI/HUD + Phase 2-5 Wiring + Phase 6 Movement System.** GameSession bootstrap wires all 10 managers, HexGrid, HUD, and SaveSystem into a playable scene. MovementManager now has Settlement Bonus (2-cycle accelerated rebuild), Salvage (Rift Shard yield from buildings on departure), and Ghost Footprint (ACTIVE→INACTIVE hex state transitions). 769 GUT tests passing across 50 test files (49 unit + 1 integration). Next: playtest, then Fog of War reveal, Movement triggers, or Endgame.
...
| 2026-02-24 | **GDD Audit Round 2 COMPLETE.** 14 fixes across 13 GDD files. Part A (6 doc fixes): era threshold ScenarioData ref, consecutive pollution clarification, Mandate Migration cost "AND" clarification, Codex unlock conditions, biome metric magnitudes, Rocky pathing quantification. Part B (8 design clarifications): Defense derived score note in Utility AI, Wave transit Anxiety trade-off, Settlement Bonus × Era stacking, quest rejection -3 morale, Festival 1-cycle cooldown gap, protest quest definition, Archive Vault Sovereign Quest specifics, Rift Density definition in Hex Terrain. |
| 2026-02-24 | **GDD update: Wave Defense System.** 5 files updated: The Wave (Defense Framework 3-layer model, Wave Intelligence System, Terrain Exploitation), Quest System (Offensive Pre-Wave Quests — 4 faction proposals), Building Evolution (Weapon Buildings — Tesla Coil, Rift Ward, Siege Ballista, Entropy Spire, updated MVP asset table 22→30 meshes), Design Pillars (Pillar 4 expanded with 3-timepoint agency + 3-layer defense), Utility AI (weapon building zone preferences, threat-aware placement rules, updated Available Types table). All cross-references verified consistent. |
| 2026-02-24 | **GDD review: Wave Defense consistency check.** No contradictions found. 1 gap identified: weapon building costs missing from WT - Economy & Resources.md. Fixed duplicate reference in Utility AI. All wiki-links valid, weapon names consistent across 5 files, master GDD 19 sections complete. |
| 2026-02-24 | **GDD fix: Weapon building costs.** Added Tesla Coil (15G/5M), Rift Ward (5G/15M), Siege Ballista (25G/8M), Entropy Spire (8G/25M) to WT - Economy & Resources.md building cost table. Symmetric Mirror structure (Science=Gold-heavy, Magic=Mana-heavy). |
| 2026-02-24 | **Weapon Buildings + AI Scoring IMPLEMENTED.** 4 weapon building .tres (Tesla Coil, Rift Ward, Siege Ballista, Entropy Spire), WEAPON enum in BuildingType, UtilityAI expanded with threat direction bonus, weapon diversity penalty, defense need bonus, zone fill cap (70%), scar avoidance (>=0.8). UtilityAIConfig gains weapon_diversity_penalty/-max. 683 GUT tests passing across 46 test files. |
| 2026-02-24 | **Phase 2: Cross-System Wiring COMPLETE.** 5 integrations: Edict→Economy (auto-sync modifiers before income), Wave→Stability (damage fraction from WaveManager), Faction Morale→Stability (4-faction threshold check), Festival→Stability (edict-driven bonus), Win Conditions (ScenarioData checked at EVOLVE end). EdictManager faction reactions fixed to use QuestManager.push_faction_morale(). EventBus gains game_won signal. 701 GUT tests passing (18 new cross-system tests across 4 test files). |
| 2026-02-24 | **Phase 3: Wave Intelligence + Offensive Quests COMPLETE.** WaveIntel RefCounted (4-level intel: BLIND→PARTIAL→GOOD→FULL from Observatory/Morale/Scout). 4 offensive pre-wave quests (Wall Ambush, Lens EMP, Veil Disruption, Coin Mercenary) with alignment gating. QuestManager offensive lifecycle (propose→approve→wave_ended resolve). Morale-scaled quest duration (<25: +1 cycle, >75: -1 cycle). WaveManager reads offensive effects (power_multiplier, defense_bonus, rift_spawn_multiplier). EventBus gains wave_intel_updated signal. 732 GUT tests passing (31 new tests across 6 test files, 2 new test files). |
| 2026-02-24 | **Phase 4: Main Game Scene COMPLETE.** GameSession bootstrap script (game_session.gd, ~170 lines) wires all 10 managers + HUD + SaveSystem. game_session.tscn with HexGridRenderer, HexHighlight, DirectionalLight, orbit camera. project.godot main_scene updated. Public bootstrap methods for testability. First integration test file (test_game_session.gd, 13 tests). 745 GUT tests passing across 49 test files. |
| 2026-02-24 | **Phase 5: SaveSystem Expansion COMPLETE.** SaveSerializer v2: 4 serialization gaps closed — HexCell.zone_type, QuestManager._offensive_quests, WaveManager._last_intel_level/_last_intel_report. All new fields backward-compatible via `.get()` defaults. 8 new tests in test_save_serializer_phase3.gd. 753 GUT tests passing across 50 test files. |
| 2026-02-25 | **Phase 6: Movement System COMPLETE.** Settlement Bonus (2-cycle ×2/×1.5 build rate, -30%/-15% cost discount), Salvage (era-based Rift Shard yield with scar penalty + time factor), Ghost Footprint (ACTIVE→INACTIVE hex transitions). EconomyManager skips INACTIVE hexes. UtilityAI reads settlement build multiplier. BuildingManager applies cost discount. 8 files modified, 16 new tests. 769 GUT tests passing across 50 test files. |
...
- **Wave Defense 3-Layer Model**: Passive (weapon buildings + watchtowers), Systemic (metrics/faction morale → organic defense), Proactive (intelligence + offensive quests + terrain). Player agency at 3 timepoints: 2 cycles before (intelligence + edicts), 1 cycle before (offensive quests), during wave (Emergency Powers).
- **Wave Intelligence System**: 4 quality levels (Blind → Partial → Good → Full) based on Observatory ruins, Lens morale >75%, Scout quest.
- **Weapon Buildings**: Era 2: Tesla Coil (Science, AoE stun), Rift Ward (Magic, barrier). Era 3: Siege Ballista (Science, single-target high damage), Entropy Spire (Magic, DoT zone). Watchtower (Era 1, neutral) existing. AI placement: Defense Perimeter, threat-direction aware, diversity rule (no >2 same type cluster).
- **Offensive Pre-Wave Quests**: 4 faction proposals triggered by Wave Intelligence: Wall "Ambush Patrol" (-20% wave power), Lens "EMP Array" (stun Crawlers), Veil "Rift Disruption Ritual" (-30% spawn), Coin "Mercenary Contract" (+25% defense). All quest approval — consistent with Indirect Sovereignty.
- **Terrain Exploitation**: "Controlled Burn" (Veil, redirect wave via Forest hexes), "Terrain Modification" (Lens, temporary barriers on Plains). Quest approval, AI/faction chooses hex positions.
- **Weapon Building Costs (Symmetric Mirror)**: Tesla Coil 15G/5M, Rift Ward 5G/15M, Siege Ballista 25G/8M, Entropy Spire 8G/25M. Science=Gold-heavy, Magic=Mana-heavy. Era 2 total=20, Era 3 total=33.
- **MVP Asset Count**: ~30 meshes (was ~22). Added Weapon (Active): 2 types × 2 tiers × 2 alignments = 8 meshes.
...
- [x] GDD Audit: 9 contradictions/cross-ref fixes across 7 GDD files (settlement duration, scar_state thresholds, Defense derived score note, Salvage refs, Settlement Bonus refs, duplicate prompt template, Biome Seep Post-MVP tag)
